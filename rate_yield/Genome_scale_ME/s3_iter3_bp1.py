...
final_dict={}
for i in range(len(blk_met_rxn)):
 for sd in me.stoichiometric_data:
    if sd.id == blk_met_rxn[-1-i]:
        print('Blocking......',sd.id)
        for r in sd.parent_reactions:
            r.upper_bound=0
            r.lower_bound=0
        ...
        growth_list=[0.95*muopt,0.98*muopt,0.99*muopt,muopt]
        ...
        final_dict[sd.id]=target_dict

with open('s3_iter3_solution.json', 'w') as fp:
