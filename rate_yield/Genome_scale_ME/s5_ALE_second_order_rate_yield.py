...
me.unmodeled_protein_fraction=0.15  #Changing the unmodeled fraction
me.gam=23.0
me.ngam=0.0
...
gur_opt=me.solution.x_dict['EX_glc__D_e']
print('Optimal growth=', muopt,
     '  Optimal glucose uptake= ',me.solution.x_dict['EX_glc__D_e'])

sur=me.reactions.EX_glc__D_e
#glucose_rate=np.array([11.5,12,12.8,12.9,
#                       13,13.2,13.4,13.6,13.8,
#                       14,14.2,15,15.2,15.5,16,16.2]) #glucose uptake for 1.05 growth rate
glucose_rate=[10.5,11,11.5,12,12.5]
me.objective={'EX_ac_e': 1.0,'EX_glc__D_e': 0.0}
target_dict={}

for gur in glucose_rate:
    sur.upper_bound=-gur
    sur.lower_bound=-gur
    compiled_expressions=compile_expressions(me)
    #solve_at_growth_rate(me,1.05,compiled_expressions=compiled_expressions) #Solve at mu=1.05/hr
    solve_at_growth_rate(me,0.95,compiled_expressions=compiled_expressions) #Solve at mu=0.95/hr
    status=me.solution.status
    print(status)
    if status == 'optimal':
        print('  set_gur= ',gur, 
              '  glucose uptake= ',me.solution.x_dict['EX_glc__D_e'], 
              '  acetate rate=', me.solution.x_dict['EX_ac_e'],
              '  Transhydrogenase flux=', me.solution.x_dict['THD2pp_FWD_PYRNUTRANSHYDROGEN-CPLX'])

        target_dict[gur]=me.solution.x_dict
        
#with open('s5_solution.json', 'w') as fp:
with open('s5_solution_growth_0_95.json','w') as fp:
