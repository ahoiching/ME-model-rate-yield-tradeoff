...
#block ICL first, as shown in iter2
blk_met_rxn=[
 'ICL' #1
]
...
final_dict={}
for i in range(len(blk_met_rxn2)):
 blk_group=blk_met_rxn2[-i-1]
 for sd in me.stoichiometric_data:
    if sd.id in blk_group:
        for r in sd.parent_reactions:
            r.upper_bound=0
            r.lower_bound=0
 print('Blocking......',blk_group)
 solve_me_model(me, 1.5, min_mu = .1, precision=1e-6,compiled_expressions=compiled_expressions)
 #Get the maximum growth rate of the current model
 sol = me.solution
 final_dict[blk_group[0]]=sol.x_dict

with open('s3_iter4_solution.json', 'w') as fp:
